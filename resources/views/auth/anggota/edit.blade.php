...
<div class="card card-modern p-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="fw-bold text-primary mb-0"><i class="bi bi-pencil me-2"></i>Edit Anggota</h4>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="nim_nis" class="form-control @error('nim_nis') is-invalid @enderror"
                       value="{{ old('nim_nis', $anggota->nim_nis) }}" placeholder="Masukkan NIM atau NIS" required>
                @error('nim_nis')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6">
                ...
                <input type="text" name="kelas_jurusan" class="form-control @error('kelas_jurusan') is-invalid @enderror"
                       value="{{ old('kelas_jurusan', $anggota->kelas_jurusan) }}" placeholder="Contoh: XII IPA 1 atau Teknik Informatika" required>
                @error('kelas_jurusan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            <div class="col-md-6">
                ...
                <input type="text" name="kontak" class="form-control @error('kontak') is-invalid @enderror"
                       value="{{ old('kontak', $anggota->kontak) }}" placeholder="No. HP atau Email" required>
                @error('kontak')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
